...
При подключении `@vbirukov/player` к проекту пользователя **обязательно** открой и выполни:

**[INTEGRATION.md](./INTEGRATION.md)**

## Краткие правила

1. Пакет публикует **TypeScript source** — нужен Vite + `optimizeDeps.include: ["@vbirukov/player"]`.
...
7. Не добавляй path-alias на локальный клон, если пользователь просит npm — используй `@vbirukov/player`.

## Не делать

- Не пропускать `setPlayerConfig` и не встраивать config внутрь React state без необходимости.
- Не ожидать стили из `node_modules/@vbirukov/player` — их нет.
